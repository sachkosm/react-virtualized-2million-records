...
*/
  function init(options) {
    ...
    if (options.rowLabels !== undefined)
      setRowLabelDisplayFields(options.rowLabels);
    if (options.columnLabels !== undefined)
      setColumnLabelDisplayFields(options.columnLabels);
    if (options.summaries !== undefined)
      setSummaryDisplayFields(options.summaries);

    if (options.csv !== undefined) processCSV(options.csv);
    if (options.json !== undefined) processJSON(options.json);
...
*/
  function config(showFields) {
    var fields;
    if (showFields === undefined) fields = cloneFields();
  ...
  function pad(sideToPad, input, width, padString) {
    ...
    while (input.length < width) {
      if (sideToPad === "left") input = padString + input;
      else input = input + padString;
    }
  ...
  function formatDate(value) {
    return (
      ...
      "-" +
      padLeft(value.getUTCMonth() + 1, 2, "0") +
      "-" +
      padLeft(value.getUTCDate(), 2, "0")
    );
  }
  ...
  function formatTime(value) {
    return (
      formatDate(value) +
      " " +
      padLeft(value.getUTCHours(), 2, "0") +
      ":" +
      padLeft(value.getUTCMinutes(), 2, "0")
    );
  }

  function isArray(arg) {
    if (!Array.isArray) return objectType(arg) == "array";
    else return Array.isArray(arg);
  }
  ...
  function processJSON(input) {
    ...
    var o = {},
      ...
      m = input.length;
    while (++j < m) {
      if (j === 0) header = processHeaderRow(input[j]);
      else rawData.push(processRow(input[j], header, pseudoFields));
    }
  }
  ...
  function processCSV(text) {
    ...
    rawData = processRows(text, function(row, i) {
      if (i === 0) header = processHeaderRow(row);
      else return processRow(row, header, pseudoFields);
    });
  }
    ...
    for (field in restrictions) {
      if (restrictions.hasOwnProperty(field))
        if (isRegExp(restrictions[field])) {
          ...
          while (++i < m) {
            restrictions[field][i] = castFieldValue(
              field,
              restrictions[field][i]
            );
          }
        } else {
  ...
  */
    ...
    while (++i < dataToFilterLength) {
      ...
      for (var key in filters) {
        if (
          ...
          matchesFilter(filters[key], row[key])
        )
          matches += 1;
      }
  ...
  function preserveFilteredData() {
    ...
    for (var key in dataFilters) {
      if (
        dataFilters.hasOwnProperty(key) &&
        dataFilters.hasOwnProperty(key) &&
        filters[key] === dataFilters[key]
      )
        matches += 1;
    }
  ...
  function getFields() {
    ...
    for (var key in fields) {
      if (fields.hasOwnProperty(key))
        retFields[fields[key].index] = fields[key];
    }
  ...
  */
  function restrictFields(type) {
    var retFields = [];
    for (var key in fields) {
      if (fields.hasOwnProperty(key) && fields[key][type] === true)
        retFields.push(fields[key]);
    }
  ...
  */
  function appendField(field) {
    ...
    if (
      field.summarizable &&
      (field.rowLabelable || field.columnLabelable || field.filterable)
    ) {
      ...
      if (summarizable_field.summarizable !== true)
        summarizable_field.name =
          summarizable_field.name + "_" + summarizable_field.summarizable;
      else summarizable_field.name = summarizable_field.name + "_count";
  ...
  */
  function addFieldValue(field, value) {
    if (fields[field] === undefined || fields[field].filterable === false)
      return;
  ...
  */
  function castFieldValue(fieldName, value) {
    ...
    switch (field.type) {
      case "integer":
      case "cents":
        if (objectType(value) === "number") return value;
 ...
 */
  ...
  */
  function appendDisplayField(type, field) {
    if (objectType(field) === "string") field = fields[field];
    ...
    while (++i < m) {
      ...
      for (var key in displayFields.rowLabels) {
        if (displayFields.rowLabels.hasOwnProperty(key)) {
          if (i === 0)
            resultsColumns.push({ fieldName: key, width: 1, type: "row" });
      ...
      if (results[resultKey] === undefined) {
        ...
        for (var key in displayFields.rowLabels) {
          if (displayFields.rowLabels.hasOwnProperty(key))
            results[resultKey][key] = fields[key].displayFunction(
              row[key],
              key
            );
        }
      ...
      if (displayFields.columnLabels.hasOwnProperty(key)) {
        ...
        for (var resultKey in results) {
          ...
          for (var value in values) {
            ...
            else columnLabelColumns[value] += 1;
